...
function reverseNumber(num) {
  // Convert the number to a string, split it into an array, reverse the array, and join it back into a string
  let reversedString = num.toString().split("").reverse().join("");

  // Convert the reversed string back to a number
  return parseInt(reversedString) * Math.sign(num); // Handles negative numbers correctly
}

let x = 32243;
let reversedNumber = reverseNumber(x);
console.log(x);
